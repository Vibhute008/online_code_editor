...
const Home = () => {
  ...
  const createProj = (e) => {
    if (projTitle === '') {
      toast.error('Please enter a project title'); // Error toast
    } else {
      fetch(api_base_url + '/createProject', {
        ...
        .catch(() => toast.error('An unexpected error occurred')); // Catch block for errors
    }
  };
    ...
    fetch(api_base_url + '/getProjects', {
      ...
      .then((data) => {
        if (data.success) {
          ...
          toast.error(data.message); // Error toast for project fetch
        }
      })
      .catch(() => toast.error('Failed to load projects')); // Catch block for errors
  };
    ...
    fetch(api_base_url + '/getUserDetails', {
      ...
      .then((data) => {
        if (data.success) {
          ...
          toast.error(data.message); // Error toast for user fetch
        }
      })
      .catch(() => toast.error('Failed to load user details')); // Catch block for errors
  }, []);
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-[100px] my-[40px] mt-[9rem]">
      ...
      {/* Project Display */}
      <div className="cards min-h-[66vh]">
        {isGridLayout ? (
